...
def patch_cli(theme):
    ...
    # 5e. Logo 三段渐变插桩（F-06）：替换 Ent() 的离散 6 色数组为逐行 lerp 渐变
    #     注入 __mcodeLogoGradient(rows, top, mid, bottom)，端点取主题三键。
    m_ent = re.search(
        r'function Ent\(t\)\{let\{fullMinWidth:e,mediumMinWidth:r,microMinWidth:i,'
        r'fallbackTitle:n\}=xS\.hero,',
        content)
    if m_ent:
        if '__mcodeLogoGradient' not in content:
            grad_fn = (
                'function __mcodeLogoGradient(rows,top,mid,bottom){'
                'var N=Math.max(1,rows),cl=(process.stdout&&process.stdout.isTTY)?'
                '(process.stdout.getColorDepth?process.stdout.getColorDepth():8):8,'
                'full=cl>=24,out=[],i,t,f,c,hex,ch;'
                ...
                'c=t<0.5?lerp(A,B,t*2):lerp(B,C,(t-0.5)*2);'
                'if(full){hex=fmt(c);out.push("\\x1b[38;2;"+c.x+";"+c.y+";"+c.z+"m")}'
                'else{out.push("\\x1b[38;5;"+idx256(c)+"m")}}return out}'
            )
            # 注入到 5a 锚点后
            inject_anchor = re.search(
                r'var __mcodePlanThemeActive=!1,__mcodePlanTheme=[^;]*?,__mcodeCurrentTheme=[^;]*?;',
            ...
            if inject_anchor:
                content = content.replace(inject_anchor.group(0),
                                          inject_anchor.group(0) + grad_fn, 1)
        # 替换 Ent 的 l 数组
        old_l = ('l=[me.wordmarkHighlight,me.wordmarkHighlight,me.brand,me.brand,'
                 'me.wordmarkShadow,me.wordmarkShadow]')
        new_l = ('l=__mcodeLogoGradient(a.length,me.wordmarkHighlight,me.brand,'
                 'me.wordmarkShadow)')
        if old_l in content:
            content = content.replace(old_l, new_l, 1)
        # 每行用渐变序列号（m 取 l[m]，l 长度=行数，无需 %7；保留 fallback）
        old_map = ('m=d%7,p=s?l[m]??me.brand:me.brand;return dun(ye.bold.hex(p)(u),t)')
        new_map = ('m=d%7,p=s?(l[m]??me.brand):me.brand;return p&&p.startsWith("\\x1b")?'
    ...
    if os.path.isfile(CLI_PATH) and os.environ.get("MCODE_THEME_NO_CHECK") != "1":
        ...
        candidates = [
            os.path.join(MCODE_LIB, "..", "..", "runtime", "node", "bin", node_exe),
            shutil.which("node"),
