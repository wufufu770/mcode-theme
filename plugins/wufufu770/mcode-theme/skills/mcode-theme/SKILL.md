...
mcode-theme install <theme.json|URL>  # 校验（9 条纪律）并安装主题包
...
2. **恢复默认**：`mcode-theme restore`。
3. **在线安装**：`mcode-theme install https://example.com/theme.json`，未通过 9 条纪律校验会拒绝并输出明细。
...
- 50 个内置主题全部通过 validate-themes.py 9 条纪律校验（亮度全序/对比度/色相/饱和度/品牌色族/三段渐变）。
