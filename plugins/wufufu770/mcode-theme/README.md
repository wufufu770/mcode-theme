...
让用户自定义 MiniMax Code CLI（mcode）的界面主题：50 个内置主题（含 Catppuccin、
Synthwave、Tokyo Night 等社区经典）、一键应用/恢复、Web 可视化配置器、主题包
导出/导入、9 条纪律自动校验、mcode 升级检测。

## 功能

- `mcode-theme list / apply / restore / current / plan` — 主题管理
- `mcode-theme web` — Web 可视化配置器（实时预览 + 真实会话预览 + 主题包导入导出）
- `mcode-theme install <theme.json|URL>` — 校验（9 条纪律）后安装
- `mcode-theme update` — 检测 mcode 升级并提示重新应用
- 50 个内置主题全部通过纪律校验（docs/validation-report.md）
